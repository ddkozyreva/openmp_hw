...
{
    // f, x, nf, nx (как и созданные ниже k, omega, но не i) по умолчанию shared
    ...
    #pragma omp parallel
    for (i=0; i < nx; i++) {  // i по умолчанию private, так как внешний цикл для pragma
    // Для параллельных процессах разные omega, k. Потому делаем их private - то есть
    // для каждого процесса создается копия данной переменной и используется эта копия в каждом процессе
        #pragma omp for private(omega) nowait  // nowait - так как не ждем когда все распараллеленные  итерации завершатся
        for (k = 0; k < nf; k++) {
            omega = 2 * PI * k / nx;
            f(k) += x(i) * cos(omega * i);
        }
